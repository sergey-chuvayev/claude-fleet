...
window.FleetTeams=(()=>{
  ...
  function render() {
    ...
    const roles=Object.entries(draft.roles),ownerReview=draft.workflow.mode==='owner-review'
    document.getElementById('team-add-role').disabled=ownerReview
    document.getElementById('team-editor-fields').innerHTML=`<div class="team-meta">${field('Team name','name',draft.name,80)}${field('Team ID','id',draft.id,60)}${field('Purpose','description',draft.description,500)}</div><div class="team-rules"><label>${ownerReview ? 'Reviewed implementations per request' : 'Repair attempts per task'}<input data-team-field="maxAttempts" type="number" min="1" max="10" value="${draft.workflow.maxAttempts}"></label><label>Usage cap · API-rate equivalent<input data-team-field="budgetUsd" type="number" min="0.1" max="1000" step="0.1" value="${draft.workflow.budgetUsd}"></label></div><p class="note">The usage cap is not a bill. It’s the API-rate equivalent the SDK reports, accumulated across the whole initiative; on a Claude subscription nothing is charged. Reaching the cap still stops the run.</p><p class="note">${ownerReview ? 'One owner implements and finishes the request in the same session. One independent reviewer checks the committed changes. The initial review counts toward the request limit.' : 'One manager talks to you. Verification roles check every task; at least one separate worker does the work.'} Shell access permits commands and is governed by your approval mode.</p><div class="team-role-list">${roles.map(([key,role],index)=>`<details class="team-role" data-role-key="${escape(key)}" ${index===0 ? 'open':''}><summary><strong>${escape(key)}</strong><span>${escape(role.description)}</span><small>${escape(role.model)}</small></summary><div class="team-role-fields"><label>Role ID<input data-role-field="id" value="${escape(key)}" maxlength="40" required></label><label>Model<input data-role-field="model" value="${escape(role.model || 'inherit')}" list="team-model-options" maxlength="80" required></label><label>Turn limit<input data-role-field="maxTurns" type="number" min="1" max="100" value="${role.maxTurns ?? (key===draft.manager ? 100:30)}" required></label><label>Reasoning effort<select data-role-field="effort">${['low','medium','high','xhigh','max'].map(e=>`<option value="${e}" ${e===(role.effort || (key===draft.manager ? 'high':'medium')) ? 'selected':''}>${e}</option>`).join('')}</select></label><label class="team-wide">Purpose<input data-role-field="description" value="${escape(role.description)}" maxlength="500" required></label><label class="team-wide">Instructions<textarea data-role-field="prompt" rows="6" maxlength="12000" required>${escape(role.prompt)}</textarea></label><div class="team-role-kind team-wide"><label><input type="radio" name="team-manager-role" data-role-field="manager" ${key===draft.manager ? 'checked':''}> ${ownerReview ? 'Owner' : 'Manager'}</label><label><input type="checkbox" data-role-field="reviewer" ${draft.workflow.reviewers.includes(key) ? 'checked':''}> Required verifier</label><button type="button" class="button" data-remove-role="${escape(key)}" ${ownerReview ? 'disabled':''}>Remove role</button></div><fieldset class="team-wide"><legend>Allowed tools</legend><div class="team-tools">${tools.map(tool=>`<label><input type="checkbox" data-tool="${escape(tool)}" ${(role.tools || []).includes(tool) ? 'checked':''}> ${escape(tool)}</label>`).join('')}</div></fieldset></div></details>`).join('')}</div><datalist id="team-model-options"><option value="opus"><option value="sonnet"><option value="haiku"><option value="inherit"></datalist>`
  ...
  function syncTools() {
    for(const row of document.querySelectorAll('#team-editor [data-role-key]')) {
      ...
      for(const el of row.querySelectorAll('[data-tool]')) {
        el.disabled=(manager && draft.workflow.mode!=='owner-review' && !['Read','Glob','Grep','WebSearch','WebFetch'].includes(el.dataset.tool)) || (verifier && ['Write','Edit','MultiEdit','NotebookEdit'].includes(el.dataset.tool))
        if(el.disabled)el.checked=false
      }
    }
  }
  function read() {
    const next={roles:{},workflow:{...(draft.workflow.mode ? {mode:draft.workflow.mode}:{}),reviewers:[]}}
    ...
    for(const row of document.querySelectorAll('#team-editor [data-role-key]')) {
      ...
      next.roles[id]={description:value('description').value.trim(),prompt:value('prompt').value.trim(),model:value('model').value.trim(),maxTurns:Number(value('maxTurns').value),effort:value('effort').value,tools:[...row.querySelectorAll('[data-tool]:checked')].map(el=>el.dataset.tool)}
  ...
  async function adjustLimits(id){
    ...
    box.innerHTML=`<label>Usage cap · API-rate equivalent<input type="number" data-limit="budgetUsd" min="0.1" max="1000" step="0.1" value="${s.limits?.budgetUsd ?? s.teamSnapshot.workflow.budgetUsd}"></label><label>${s.teamSnapshot.workflow.mode==='owner-review' ? 'Reviewed implementations per request' : 'Attempts per task'}<input type="number" data-limit="maxAttempts" min="1" max="10" value="${s.limits?.maxAttempts ?? s.teamSnapshot.workflow.maxAttempts}"></label><button type="button" class="button">Save limits</button><p class="note">Not a bill: the SDK reports this as an API-rate equivalent, and a Claude subscription is charged nothing. The cap still stops the run.</p><p class="form-error" role="alert" hidden></p>`
